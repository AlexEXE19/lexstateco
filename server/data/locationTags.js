...
// `location` filter matches against Property.location.city, not
// neighborhood), so whichever tag+location the endpoint hands back is
// guaranteed to return real results when a visitor clicks through to it.
// There are only 4 real towns seeded right now, so "5 locations per tag"
// isn't honest - tags reuse the same towns under different, plausible
// themes instead.
const locationTags = {
